...
import { nodeDetailToViewModel } from './node-detail-adapter';
...
describe('nodeDetailToViewModel', () => {
  ...
  it('lists the declared interface rows', () => {
    const vm = nodeDetailToViewModel(DETAIL);
    expect(vm.declaredInterface).toEqual([
      { id: 'sA/label#', label: 'label', subKind: 'react:prop' },
    ]);
  });
