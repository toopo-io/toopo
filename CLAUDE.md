...
- **ADR-0016** — parsing & resolution (tree-sitter via `web-tree-sitter`; custom heuristic resolver; NOT stack-graphs, NOT LSP; file-level incremental). **C11 amendment**: the unresolved/ambiguous tail is persisted as a project-scoped `UnresolvedReference` sibling (never a fabricated edge) so a later "unused"/"cycle" view stays honest; **C10**: a value namespace-import member (`import * as NS; NS.foo`) resolves to the exported member through the export chain.
