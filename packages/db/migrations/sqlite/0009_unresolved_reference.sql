...
-- the honest tail of the Resolve pass — an import/usage that could not be bound
-- to a precise symbol — kept as a first-class, project-scoped sibling of the
-- graph (NOT a fabricated edge), so a later "unused"/"cycle" view never reads a
-- resolution gap as genuine absence. Mirrors the Postgres migration.
--
