...
import type { ResolvedImport, ResolverPlugin } from '../plugin/resolver-plugin.js';
 ...
 * it against the file's resolved imports. The plugin returns descriptors (target
 * `calls`/render edge, payload→prop `references`); the engine mints them and
 * never upgrades their certainty. Call-sites the parser already bound in-file
 * are skipped, so a binding is never duplicated.
 */
