...
| `packages/db` | existing | persistence: Kysely dual-backend (SQLite self-host / Postgres cloud) + Better Auth tables + project tenancy + project-scoped Serve read primitives + the persisted `unresolved_reference` tail & its honesty read primitive (ADR-0016 amendment, C11) + the `job` table & `JobStore` claim seam (ADR-0017, ADR-0020, ADR-0022, ADR-0023) |
