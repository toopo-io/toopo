-- GitHub-App connect (ADR-0026): the install flow's persistence, distinct from
-- the graph. Mirrors the Postgres migration; libSQL uses `date` columns and the
-- app supplies UTC ISO timestamps (the 0004_project convention).
--
...
--    picker filters archived rows out. Nullable; null means active.
alter table "project" add column "archived_at" date;
...
create table "github_installation" ("installation_id" text not null primary key, "owner_user_id" text not null, "created_at" date not null, "updated_at" date not null);
